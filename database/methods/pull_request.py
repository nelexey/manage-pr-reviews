...
from database.models.pull_request import PullRequest, PRStatus
...
async def create_pr(session: AsyncSession, pr_id: str, pr_name: str, author_id: str, reviewers: list[str]) -> PullRequest:
    pr = PullRequest(
...
async def get_pr(session: AsyncSession, pr_id: str) -> Optional[PullRequest]:
    stmt = select(PullRequest).where(PullRequest.pull_request_id == pr_id)
    result = await session.execute(stmt)
    return result.scalar_one_or_none()

async def merge_pr(session: AsyncSession, pr_id: str) -> Optional[PullRequest]:
    pr = await get_pr(session, pr_id)
    if pr and pr.status != PRStatus.MERGED.value:
        pr.status = PRStatus.MERGED.value
        pr.merged_at = datetime.now(timezone.utc).replace(tzinfo=None)
        await session.flush()
    return pr

async def update_pr_reviewers(session: AsyncSession, pr_id: str, reviewers: list[str]) -> Optional[PullRequest]:
    pr = await get_pr(session, pr_id)
    if pr:
