...
from database.methods.pull_request import (
    check_pr_exists, create_pr, get_pr, merge_pr, update_pr_reviewers
)
from database.methods.user import get_user, get_candidate_reviewers
from misc.schemas.pull_requests import (
    CreatePRRequest, MergePRRequest, ReassignPRRequest, 
    PRResponse, ReassignPRResponse
)
...
async def create_pull_request(payload: CreatePRRequest, session: DbSession):
    ...
    candidates = await get_candidate_reviewers(session, author.team_name, exclude_user_ids=[author.user_id])
    reviewers = random.sample(candidates, min(2, len(candidates)))
    reviewer_ids = [r.user_id for r in reviewers]
    
    pr = await create_pr(session, payload.pull_request_id, payload.pull_request_name, payload.author_id, reviewer_ids)
    await session.commit()
    
    return {"pr": {"pull_request_id": pr.pull_request_id, "pull_request_name": pr.pull_request_name, "author_id": pr.author_id, "status": pr.status, "assigned_reviewers": pr.assigned_reviewers, "createdAt": pr.created_at, "mergedAt": pr.merged_at}}
...
async def reassign_reviewer(payload: ReassignPRRequest, session: DbSession):
    ...
    if payload.old_user_id not in pr.assigned_reviewers:
        raise AppError(status_code=409, code="NOT_ASSIGNED", message="reviewer is not assigned to this PR")
    ...
    if not candidates:
        raise AppError(status_code=409, code="NO_CANDIDATE", message="no active replacement candidate in team")
        
    new_reviewer = random.choice(candidates)
    
    new_reviewers = [new_reviewer.user_id if r == payload.old_user_id else r for r in pr.assigned_reviewers]
    await update_pr_reviewers(session, pr.pull_request_id, new_reviewers)
    await session.commit()
    
    return {
        "pr": {"pull_request_id": pr.pull_request_id, "pull_request_name": pr.pull_request_name, "author_id": pr.author_id, "status": pr.status, "assigned_reviewers": pr.assigned_reviewers, "createdAt": pr.created_at, "mergedAt": pr.merged_at},
